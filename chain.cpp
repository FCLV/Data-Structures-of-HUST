...
status compare(int x1, int x2){                 //比较函数，若相同返回1，否则返回0 
	if(x1 == x2)
		return TRUE;
	else
		return FALSE;
...
status visit(LNode *a){                         //访问函数，起到打印数据项的功能 
	while(a != NULL){
		a = a->next;
		printf("%d ", a->data);
	}
...
status ClearList(LNode *L){    //清空，函数序号为3 
	...
	LNode *p = L->next;        //p从头节点开始 
	LNode *q = p->next;        //q比p快一步，每次都把p处元素清空 
	while(p){
		free(p);
		p = q;
		if(p){
			q = q->next;       //如果不是最后一项，则q后移一位 
		}
	}
	L->next = NULL;
	printf("\t线性表已清空\n");
...
status GetElem(LNode *L, int i, ElemType &e){  //查找元素值，函数序号为6 
	...
	while(p && j<i){         //移动，直至定位到指定下标或者到表尾元素 
		p = p->next;
		j++;
	}
...
status LocateElem(LNode *L, ElemType e, int (*func) (int x1, int x2)){   //查找下标，函数序号为7 
	...
	LNode *p = L->next;
	for(j=0; p; j++){
		if(func(p->data, e)){     //使用比较函数compare判断两束是否相等 
...
status ListInsert(LNode *L, int i, ElemType e){            //插入，函数序号为10 
	...
	s->data = e;
	s->next = p0->next;    //新节点指针指向第i个节点的后继 
	p0->next = s;          //第i个节点的前驱指向新节点 
	printf("\t插入成功\n");
	return OK;
}
...
status ListDelete(LNode *L, int i, ElemType &e){   //删除，函数序号为11 
	...
	if(!(p->next) || j>i-1){   //越界判断 
		printf("\t输入下标不合法\n");
		return ERROR;
	}
	
	LNode *q = p->next;  //取第i个节点 
	p->next = q->next;   //第i-1个节点指向原来第i+1个节点 
	e = q->data;         //取删除的数据 
	printf("\t元素%d删除成功\n", e); 
	free(q);
	return OK;
...
status ReadFile(LNode *L){                //读取文件，函数序号为14 
	...
	while(fread(L, sizeof(LNode), 1, fp)); //读取，直到表尾 
	printf("读取成功\n");
	fclose(fp);
	return OK; 
}
